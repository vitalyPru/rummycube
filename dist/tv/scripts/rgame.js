...
$(GameManager = function(){
	...
	function onGameStarted(clientId, game_state) {
        // Notify the clients via the Connectivity manager.
		
        ConnectivityManager.onGameStarted(clientId, game_state);
    }
    ...
    function addPlayer(clientId, name, color) {
        ...
        if (game.scene.isActive('MainMenu')) {
            game.scene.getScene('MainMenu').onPlayerUpdate(Object.keys(clientIdToSlotMap).length);
        }
...
//        }
    ...
    function onGroupMove(clientId, direction) {
		if (game.scene.isActive('PlayGame')) {
             game.scene.getScene('PlayGame').onGroupMove(clientId, direction);
        }
    }
    ...
    function onCursorToRemote(clientId, chip) {
 		if (game.scene.isActive('PlayGame')) {
             game.scene.getScene('PlayGame').onCursorToRemote(clientId, chip);
        }
    }

	function onPointerMove(clientId, pointer ) {
		if (game.scene.isActive('PlayGame')) {
             game.scene.getScene('PlayGame').onPointerMove(clientId, pointer);
        }
 	}
	
    function onRemoteClick(clientId, chip) {
 		if (game.scene.isActive('PlayGame')) {
             game.scene.getScene('PlayGame').onRemoteClick(clientId);
        }
    }
	
    function onTurnEnd(clientId) {
 		if (game.scene.isActive('PlayGame')) {
             game.scene.getScene('PlayGame').onTurnEnd(clientId);
        }
    }

    function onCursorToLocal(clientId) {
 		if (game.scene.isActive('PlayGame')) {
             game.scene.getScene('PlayGame').onCursorToLocal(clientId);
        }
    }
    ...
    return {
        ...
        onGameStarted: function(clientId, game_state) { onGameStarted(clientId, game_state); },
        ...
        // Player Control Methods
        onGroupMove: function(clientId, direction) { onGroupMove(clientId, direction ); },
        onPointerMove: function(clientId, pointer) { onPointerMove(clientId, pointer ); },
        onCursorToRemote: function(clientId, chip) { onCursorToRemote(clientId,chip); },
        onCursorToLocal: function(clientId, chip) { onCursorToLocal(clientId,chip); },
        onTurnEnd: function(clientId, msg) { onTurnEnd(clientId); },
        onRemoteClick: function(clientId, msg) { onRemoteClick(clientId); },
